...
## Intermediate: Data Cleaning Example

**Proficiency Level:** Intermediate

**Overview:**  
This .do file meticulously processes raw data from the Hurricanes & Music study. It sets up the working directory and loads the dataset, then splits string variables into numeric components. The script creates binary dummy variables for categorical responses across multiple survey sections (e.g., emotion indicators, plan for home, shelter items, kit items) and recodes open-ended emotion responses into standardized binary variables for various emotional states. These responses are then aggregated into composite emotion scales (positive, negative, and other). The file further performs a range of descriptive analyses—including chi-square tests, t-tests, and Kruskal-Wallis tests—to assess relationships among treatment conditions, behavioral outcomes, and demographics. Finally, it ensures proper variable labeling and handles missing data by recoding default responses, effectively preparing the cleaned dataset for subsequent advanced analyses.

**Key Techniques & Features:**
- **Data Import & Setup:** Loads raw data and sets the working directory.
- **String Splitting & Conversion:** Splits string variables into numeric components.
- **Dummy Variable Creation:** Generates binary indicators for multiple categorical responses.
- **Open-Ended Emotion Coding:** Standardizes open-ended emotion responses into binary variables.
- **Composite Scale Aggregation:** Aggregates emotion responses into positive, negative, and other composite scales.
- **Descriptive & Inferential Analysis:** Conducts chi-square, t-, and Kruskal-Wallis tests to assess relationships.
- **Missing Data Handling:** Recodes missing values and performs variable labeling for clarity.
...
/* Generate binary emotion variables from open-ended responses */
	...
	replace eo_other = 1 if upper(eo_oneword2) == "CALM" | upper(eo_oneword2) == "AT EASE" | upper(eo_oneword2) == "RELAXED" | upper(eo_oneword2) == "CONFUSED" | upper(eo_oneword2) == "AMAZED" | upper(eo_oneword2) == "SURPRISED" | upper(eo_oneword2) == "ASTONISHED" | upper(eo_oneword2) == "OTHER"  
...
/* Frequencies, chi-square tests, t-tests, Kruskal-Wallis */
	tab treatment makeplan, chi2  /* Outcome: makeplan vs. treatment */
	tab treatment intention, chi2  /* Outcome: intention vs. treatment */

/* Kruskal-Wallis test vs. t-test for treatment vs. intention */
	kwallis intention, by(treatment)
	ttest intention if treatment != 0, by(treatment) unequal
	ttest intention if treatment != 1, by(treatment) unequal
	ttest intention if treatment != 2, by(treatment) unequal
...
/* Demographics summary statistics */
	...
	ttest experience if treatment != 0, by(treatment) unequal
	ttest experience if treatment != 1, by(treatment) unequal
	ttest experience if treatment != 2, by(treatment) unequal
	tab country gender 
...
********** SECTION 7: VARIABLE LABELING & CLEANUP **********
	...
	/* Frequencies for open-ended emotions after categorization */
	tab eo_oneword 
	tab eo_oneword2
...
********** SECTION 8: HANDLE MISSING DATA **********
	...
	replace subscribe = 0 if missing(subscribe) /* Two missing observations = did not subscribe */
	replace makeplan = 0 if missing(makeplan) /* One missing observation recoded as no plan */
	tab makeplan
	gen intentionmiss = intention
	replace intentionmiss = . if missing(intention)
	sum intentionmiss
	replace intention = 1 if missing(intention) /* Three missing observations recoded as no intention */
	sum intention
```
...
**Insights:**  
- **Data Processing:** The script handles extensive data cleaning tasks, including splitting and recoding string variables, generating binary dummies, and aggregating responses into composite scales.
- **Emotion Coding:** Open-ended responses are standardized into binary indicators for multiple emotional states, which are then aggregated into positive, negative, and other composite scales.
- **Descriptive Analyses:** Conducts a variety of statistical tests (chi-square, t-tests, Kruskal-Wallis) to explore relationships among treatment conditions, demographics, and behavioral outcomes.
- **Variable Labeling & Cleanup:** Ensures variables are properly labeled and missing data are handled consistently.
